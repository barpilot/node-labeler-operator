...
func TestPodKillerSameSpec(t *testing.T) {
	defaultPT := &labelerv1alpha1.PodTerminator{
		Spec: labelerv1alpha1.PodTerminatorSpec{
			PeriodSeconds:      300,
			MinimumInstances:   2,
	...
	tests := []struct {
		...
		newPT *labelerv1alpha1.PodTerminator
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			pk := labeler.NewPodKiller(test.pt, mk8s, log.Dummy)
...
func TestPodKillerPodKill(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			// Call the logic to test.
			pk := labeler.NewCustomPodKiller(test.pt, mk8s, mtime, log.Dummy)
			err := pk.Start()
			defer pk.Stop()
